# SDK-REACT Web WebComponents in Next 16

Integration with Next 16
...
> [!IMPORTANT]
> **Turbopack Compatibility**: Next.js 16 uses Turbopack by default, which has compatibility issues with Stencil web components (used by Facephi SDK). The build script uses the `--webpack` flag to force webpack instead of Turbopack, which properly handles the dynamic imports required by Stencil components.

## Widget Customisation in Next 16

> [!IMPORTANT]  
> When styling Web Components in Next.js 16, consider the following:
